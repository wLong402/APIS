...
from ..repositories import ProfitsSkuRepository
from ..sdk.api import ProfitsSkuQueryAPI


class ProfitsSkuPullService(BasePullService):
    SERVICE_NAME = 'profits_sku'
    SYSTEM_NAME = 'wdt'
    API_NAME = 'profits_sku_query'

    def __init__(self, client: WdtClient = None, repository: ProfitsSkuRepository = None):
        self.client = client or get_wdt_client()
        self.repo = repository or ProfitsSkuRepository()
        super().__init__(self.client, self.repo)

    @property
    def profits_sku_api(self) -> ProfitsSkuQueryAPI:
        return self.client.profits_sku_api
    ...
    def _fetch_data(self, start_time: str, end_time: str, **kwargs) -> List[Dict]:
        ...
        call_params = dict(
            ...
            composite_dim=str(kwargs.get('composite_dim') or '1'),
            ...
            display_by_shop=kwargs.get('display_by_shop'),
            display_by_date=kwargs.get('display_by_date'),
        )
        print(f"[PARAMS DEBUG] profits_sku query_all 传参: {call_params}", flush=True)

        data = self.profits_sku_api.query_all(
            start_date=start_date,
            end_date=end_date,
            scheme_name=kwargs.get('scheme_name', '系统方案'),
            terms_income=str(kwargs.get('terms_income', '1')),
