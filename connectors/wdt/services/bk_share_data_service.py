...
from ..repositories import BkShareDataRepository
from ..sdk.api import BkShareDataQueryAPI


class BkShareDataPullService(BasePullService):
    SERVICE_NAME = 'bk_share_data'
    SYSTEM_NAME = 'wdt'
    API_NAME = 'bk_share_data_query'

    def __init__(self, client: WdtClient = None, repository: BkShareDataRepository = None):
        self.client = client or get_wdt_client()
        self.repo = repository or BkShareDataRepository()
        super().__init__(self.client, self.repo)

    @property
    def bk_share_data_api(self) -> BkShareDataQueryAPI:
        return self.client.bk_share_data_api
    ...
    def _fetch_data(self, start_time: str, end_time: str, **kwargs) -> List[Dict]:
        ...
        data = self.bk_share_data_api.query_all(
