...
class ProfitsOrderQueryAPI:
    METHOD = 'wdt.hjy.bac.profits.order.query'
    ...
    def _parse_result_data(self, data_list: List[Dict]) -> List[Dict]:
        parsed = []
        for item in data_list:
            results = item.get('result', [])
            for result_str in results:
                try:
                    columns = json.loads(result_str)
                    row = {}
                    for col in columns:
                        row[col['column']] = col['columnValue']
                    parsed.append(row)
    ...
    def query(
        ...
        plat_order_nos: Optional[str] = None,
        erp_order_nos: Optional[str] = None,
        order_way: Optional[str] = None,
        cost_type: Optional[str] = None,
        stat_mode: Optional[str] = '1',
        composite_dim: Optional[str] = None,
        ...
        sign_params = {
            ...
            'splitSuite': str(split_suite),
            'originalOrder': str(original_order),
        }

        if shop_nos:
            sign_params['shopNos'] = shop_nos
        if plat_order_nos:
            sign_params['platOrderNos'] = plat_order_nos
        if erp_order_nos:
            sign_params['erpOrderNos'] = erp_order_nos
        if order_way is not None:
            sign_params['orderWay'] = str(order_way)
        if cost_type is not None:
            sign_params['costType'] = str(cost_type)
        if stat_mode is not None:
            sign_params['statMode'] = str(stat_mode)
        if composite_dim is not None:
            sign_params['compositeDim'] = str(composite_dim)
        ...
        if debug:
            ...
            debug_print(f"      [URL DEBUG] sys_params: {sys_params}")
    ...
    def query_all(
        ...
        while True:
            ...
            result = self.query(
                ...
                shop_nos=shop_nos,
                plat_order_nos=plat_order_nos,
                erp_order_nos=erp_order_nos,
                order_way=order_way,
                cost_type=cost_type,
                stat_mode=stat_mode,
            ...
            if data_list:
                parsed = self._parse_result_data(data_list)
